...
def setting_section(resolution_width, resolution_length):
	section[0] = Section(resolution_width/2, resolution_width, 0, resolution_length)
	section[1] = Section(0, resolution_width/2, 0, resolution_length)
...
def format_for_audrino():
    format = ';'
    for i in range(0,2):
        if section[i].get_color() != None:
            format += section[i].get_ascii_color()
        else:
            format += 0
        if i is 1:
            format += ','
